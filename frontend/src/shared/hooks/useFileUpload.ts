...
import {
  startUpload, setProgress, finishUpload, failUpload,
} from "@/features/complaints/uploadSlice";
...
export const ACCEPTED_EXTENSIONS =
  ".pdf,.jpg,.jpeg,.png,.gif,.webp,.txt,.eml,.doc,.docx";
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        xhr.addEventListener("load", () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            dispatch(
              uploadApi.util.invalidateTags([{ type: "Upload", id: complaintId }])
            );
