...
import type { DashboardStats, Complaint, ComplaintListResponse } from "@/shared/types";
...
export const fetchDashboard = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const { data } = await api.get<ComplaintListResponse>("/complaints", {
...
export function computeStats(complaints: Complaint[]): DashboardStats {
  return {
...
const dashboardSlice = createSlice({
  ...
  reducers: {
    updateDashboard: (
      state,
      { payload }: PayloadAction<{ complaints: Complaint[]; total: number }>
    ) => {
