...
export const uploadApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    uploadDocument: build.mutation<UploadedDocument, { complaintId: string; file: File }>({
    ...
    getDocuments: build.query<UploadedDocument[], string>({
      query: (complaintId) => `/complaints/${complaintId}/documents`,
      providesTags: (_r, _e, id) => [{ type: "Upload", id }],
    }),

    deleteDocument: build.mutation<void, { complaintId: string; documentId: string }>({
      ...
      onQueryStarted: async ({ complaintId, documentId }, { dispatch, queryFulfilled }) => {
        const patch = dispatch(
          uploadApi.util.updateQueryData("getDocuments", complaintId, (draft) => {
